#include <commonstrings.h>

extern "C" {

const char* const doneStr = "done";
const char* const ellipsisStr = "...";
const char* const englishCapitalLetters = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
const char* const englishSmallLetters = "abcdefghijklmnopqrstuvwxyz";
const char* const failedStr = "failed";
const char* const falseStr = "false";
const char* const hexPalette = "0123456789abcdef";
const char* const notStr = "not";
const char* const okStr = "ok";
const char* const outOfMemoryStr = "Out of memory";
const char* const pagesAvailableStr = "Pages available ";
const char* const presentStr = "present";
const char* const trueStr = "true";

}
